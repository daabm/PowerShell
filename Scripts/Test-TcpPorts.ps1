<#
        ...
        This basic check ensures that at least no firewalls or stale DNS records are causing issues. All port checks are executed in parallel which greatly improves total processing time over Test-NetConnection or PortQry.exe.
        ...
        The results are collected in an array of [PSCustomObject]. This array is piped to Out-Gridview for quick convenient analysis.

        The columns of the results are mostly self explaining. Since we do a lot of DNS resolution which for cnames can "change" the computername, the original value that led us to each IP Address is also preserved in the results.
        ...
        Since all required parameters are pipeline aware, you can also pipe an array of objects to the script. These objects must have the required properties, at least 'Computer'. Optionally, you can use 'Ports', 'SSLPorts' and other properties. This enables you to quickly test an array of computers where each computer can also be tested for different ports and with different additional parameters.
        ...
        .PARAMETER EnableCertValidation

        By default, certificate errors are ignored. This prevents needing all possible root and ca certificates in the local certificate store. If you want to examine SSL certificates, you can do so afterwards -PassThru. Each object in the results has a .certificate property which you can then test with Test-Certificate.
        If you want the Windows libraries to do certificate validation, use this switch. Be aware that this will cause all SSL handshakes to fail if the cert chain is not trusted, despite any other cert errors like invalid names or expired certs.
        ...
        By default, the results are piped to Out-GridView only, and nothing is returned. Add this switch if you want to store the resulting array for further processing.

        .EXAMPLE

        .\Test-TcpPorts.ps1 -IncludeEPM

        Verifies the domain of the current computer, evaluates all IP addresses and verifies a pre defined port list against these. It also checks all dynamic RPC endpoints.

        .EXAMPLE

        'corp','tailspintoys.com' | .\Test-TcpPorts.ps1 -DNSSuffix 'contoso.com'

        Enumerates all IP Addresses for the provided names after appending the DNSSuffix 'contoso.com' to 'corp' and verifies a pre defined port list against these. Since these are domain names, they will resolve to IP addresses of all domain controllers.
        ...
        .\Test-TcpPorts.ps1 'Computer1','Computer2' -Ports 80,443 -ResolvePortNames -VerifySSL -SSLPorts 443
        ...
        .\Test-TcpPorts.ps1 -VerifySSL -PassThru | Foreach { $_.Certificate.Save( $env:Userprofile ) }

        Verifies all Domain Controllers of the current domain including SSL for ports 636 and 3269. Then saves the certificates found in the user's profile directory.
        ...
        To re-use the certificates, they offer a .Save( TargetPath [, OverwriteExisting] ) method. The filenames are derived from the certificate subject. You can then simply pipe the PassThru output into a foreach loop, see examples.
...
Begin {
    ...
    Import-Module -Name DnsClient -ErrorAction Stop -Verbose:$false

    # if we have a source computer, save the currently running code to a scriptblock.
    # we could also read the script file, but if you copy it to your PS interpreter and run it directly,
    # there will be no script file...

    If ( $SourceComputer ) {
        $TestTcpConnections = $MyInvocation.MyCommand.ScriptBlock
        $ScriptBlockInvokeCommand = {
            $Computer = $args[0]
            $ScriptBlockContent = $args[1]
            $BoundParameters = $args[2]
            $ScriptBlockTestTcpPorts = [Scriptblock]::Create(@"
            &{ $($ScriptBlockContent) } $($BoundParameters )
"@)
            $Session = New-PSSession -ComputerName $Computer
            $Result = Invoke-Command -Session $Session -ScriptBlock $ScriptBlockTestTcpPorts
...
'@
            ...
            If ( $Success ) {
                Foreach ( $Protocol in $Results ) {
                    Try {
                        ...
                        If ( $EnableCertValidation ) {
                            $SslStream = [Net.Security.SslStream]::new( $NetStream, $true )
                        } Else {
        ...
        $ScriptBlockSaveCert = {
            ...
            $InsertLineBreaks = 1
            $CertData = $This.RawData
            $CertName = ( $This.SubjectName.Name.SubString( 3 ) -split ',' )[0]
            $TargetFile = "$TargetPath\$CertName.cer"
            $oPem = [Text.StringBuilder]::new()
            [void] $oPem.AppendLine( '-----BEGIN CERTIFICATE-----' )
            [void] $oPem.AppendLine( [Convert]::ToBase64String( $CertData, $InsertLineBreaks ) )
            [void] $oPem.AppendLine( '-----END CERTIFICATE-----' )
            $oPem.ToString() | Out-File $TargetFile -Force:$Force
        }
    ...
    function Resolve-DnsHost {
        ...
        If ( [URI]::CheckHostName( $HostEntry ) -eq 'Dns' ) { # if we got a name, use dns resolution to get IP addresses
            If ( $HostEntry -notmatch '\.' ) { $HostEntry = "$HostEntry.$DNSSuffix" } # add global dns suffix if host does not contain a dot
            Try {
                $DNSEntries = @( Resolve-DnsName $HostEntry -Type A -ErrorAction Stop | Where-Object { $_.Type -eq 'A' } )
            } Catch {
                Write-Warning "Could not resolve host $HostEntry - host will not be checked!"
                $DNSEntries = @()
            }
            Foreach ( $DNSEntry in $DNSEntries ) {
                [void] $Result.Add( [PSCustomObject] @{
                        OriginalName = $HostEntry
                        IPAddress   = [String] $DNSEntry.IPAddress
                        Domain      = $Domain
                } )
            }
        } Else { # if we did NOT get a name, treat it as an IP address - no validation, could be IPv4/IPv6 with any of its shortenings which is hard to validate
            [void] $Result.Add( [PSCustomObject] @{
                    OriginalName = $HostEntry
...
Process {
    # . .\Test-TcpPorts.ps1 -Computer idfi.rz.bankenit.de -SourceComputer ( Get-ADDomain -Current LocalComputer ).ReplicaDirectoryServers

    If ( $SourceComputer ) {
        $SourceComputerList = [Collections.ArrayList]::new()
        Foreach ( $c in $SourceComputer ) {
            $SourceComputerList.AddRange( @( ( Resolve-DnsHost -HostEntry $c ) ) )
        }
        Foreach ( $SourceC in $SourceComputerList ) {
            If ( -not $PassThru ) {
                Write-Host "Preparing PSSession for target $( $SourceC.OriginalName )..."
            }
            $ScriptBlockParams = ''
            # add all unnamed params
            Foreach ( $Parameter in $args ) {
                $ScriptBlockParams += "$Parameter "
            }
            # add all named params except SourceComputer and PassThru
            Foreach ( $Parameter in $PSBoundParameters.Keys ) {
                If ( $Parameter -ne 'SourceComputer' -and $Parameter -ne 'PassThru' ) {
                   $Value = $PSBoundParameters[ $Parameter ]
                   If ( $Value -eq $true ) {
                    $ScriptBlockParams += "-$parameter "
                   } Else {
                    $ScriptBlockParams += "-$Parameter $Value "
                   }
                }
            }
            $ScriptBlockParams += '-PassThru' #need this because we are running everything in PSSession which does not support Out-GridView
            $Job = New-RunspaceJob -Pool $RunspacePool -ScriptBlock $ScriptBlockInvokeCommand -Arguments @( $SourceC.OriginalName, $TestTcpConnections, $ScriptBlockParams )
            $SourceC | Add-Member -NotePropertyName 'Job' -NotePropertyValue $Job
        }
        Foreach ( $SourceC in $SourceComputerList ) {
            Write-Host "Retrieving results for target $( $SourceC.OriginalName )..."
            If ( $SourceC.Job.Powershell.HadErrors ) {
                $SourceC.Job.Powershell.Streams.Error
            } Else {
                If ( $VerbosePreference -eq 'Continue' ) {
                    $SourceC.Job.Powershell.Streams.Verbose
                }
                $JobResults = $SourceC.Job.Powershell.EndInvoke( $SourceC.Job.AsyncResult )
                Foreach ( $JobResult in $JobResults ) {
                    $JobResult = $JobResult | Select-Object -Property * -ExcludeProperty RunspaceID, PSShowComputername
                    [void] $ResultList.Add( $JobResult )
                }
            }
        }
        ...
        If ( -not $PassThru ) { Write-Host "Initiating ICMP connection test for $( $ComputerList.Count ) computers." }
        ...
        If ( -not $PassThru ) { Write-Host 'Collecting ICMP connection test result for all computers.' }
        ...
        If ( -not $PassThru ) { Write-Host "Detaching port connection test for $( $ComputerList.Count ) computers." }
        $JobsTotal = 0
        ...
        Foreach ( $HostEntry in $ComputerList ) {
            If ( $IgnoreICMP -or ( $HostEntry.ICMP -eq 'Success' ) ) {
                ...
                If ( $IncludeEPM -or $EPMOnly ) {
                    ...
                    $ArgumentList = @( $HostEntry.IPAddress, $Timeout, $EnableCertValidation )
        ...
        If ( -not $PassThru ) { Write-Host "Detached $JobsTotal background jobs successfully." }
        If ( -not $PassThru ) { Write-Host "Collecting port connection test result for $( $ComputerList.Count ) computers." }
        ...
        Foreach ( $HostEntry in $ComputerList ) {
            ...
            Foreach ( $Port in $HostEntry.Ports | Where-Object { $_.SSLJob }) {
                ...
                If ( $Certificate ) {
                    ...
                    $HostEntry.Certificate | Add-Member -MemberType ScriptMethod -Name 'Save' -Value $ScriptBlockSaveCert -Force
                }
            }
            If ( $HostEntry.EPMJob ) {
